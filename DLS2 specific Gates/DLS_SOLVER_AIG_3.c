...
/* ============================================================
 * HIGHWAY ROUTING STRUCTURES (NEW)
 * ============================================================ */

#define MAX_HIGHWAYS 256
#define MAX_GATES_PER_COLUMN 128
#define HIGHWAY_LANE_WIDTH 0.25f

typedef struct {
    float y;
    int lane_count;
    float lane_offsets[32];
} Highway;

typedef struct {
    Highway highways[MAX_HIGHWAYS];
    int num_highways;
    float min_y, max_y;
} HighwayNetwork;
...
typedef struct {
    ForbiddenZone zones[MAX_GATES_PER_COLUMN];
    int zone_count;
} ColumnObstacles;
...
static LayoutConfig get_default_layout_config(void) {
    return (LayoutConfig){
        .gate_width = 0.8f,
        .gate_height = 0.8f,
        .column_spacing = 14.0f,    // Gives room for the S-curves
        .alley_width = 8.0f,        // Wide enough for 15 lanes
        .min_gate_v_spacing = 5.0f, // Prevents gates from touching
        .lane_spacing = 0.30f,      // Perfect gap between wires
        .input_v_spacing = 1.2f,
        .output_v_spacing = 1.2f,
        .pin_margin_x = 1.0f,
        .chip_padding = 2.0f,
        .chip_color_r = 0.12f,      // Dark theme looks great in DLS2
        .chip_color_g = 0.14f, 
        .chip_color_b = 0.18f
    };
}
...
/* ============================================================
 * HIGHWAY ROUTING FUNCTIONS (NEW)
 * ============================================================ */

static void build_column_obstacles(
    Circuit *c,
    int *depths,
    float *gate_y,
    LayoutConfig *cfg,
    int max_depth,
    ColumnObstacles *obstacles
) {
    for (int col = 0; col <= max_depth; col++) {
        obstacles[col].zone_count = 0;
    }
    
    float gate_half_height = cfg->gate_height / 2.0f + cfg->lane_spacing;
    
    for (int i = 0; i < c->num_gates; i++) {
        if (!c->gates[i].alive) continue;
        
        int col = depths[i];
        if (col < 0 || col > max_depth) continue;
        
        int idx = obstacles[col].zone_count;
        if (idx >= MAX_GATES_PER_COLUMN) continue;
        
        obstacles[col].zones[idx].y_min = gate_y[i] - gate_half_height;
        obstacles[col].zones[idx].y_max = gate_y[i] + gate_half_height;
        obstacles[col].zone_count++;
    }
    
    for (int col = 0; col <= max_depth; col++) {
        ColumnObstacles *obs = &obstacles[col];
        for (int i = 0; i < obs->zone_count - 1; i++) {
            for (int j = i + 1; j < obs->zone_count; j++) {
                if (obs->zones[j].y_min < obs->zones[i].y_min) {
                    ForbiddenZone tmp = obs->zones[i];
                    obs->zones[i] = obs->zones[j];
                    obs->zones[j] = tmp;
                }
            }
        }
...
static bool is_y_clear_globally(
    float y,
    ColumnObstacles *obstacles,
    int num_columns,
    float clearance
    ...
    for (int col = 0; col < num_columns; col++) {
        ColumnObstacles *obs = &obstacles[col];
        for (int z = 0; z < obs->zone_count; z++) {
            if (y >= (obs->zones[z].y_min - clearance) &&
                y <= (obs->zones[z].y_max + clearance)) {
                return false;
            }
        }
    }
...
static void compute_highway_network(
    Circuit *c,
    int *depths,
    float *gate_y,
    int num_inputs_local,
    float *input_y,
    int num_outputs_local,
    float *output_y,
    LayoutConfig *cfg,
    int max_depth,
    HighwayNetwork *network
) {
    network->num_highways = 0;
    
    ColumnObstacles *obstacles = (ColumnObstacles*)calloc(max_depth + 2, sizeof(ColumnObstacles));
    if (!obstacles) return;
    
    build_column_obstacles(c, depths, gate_y, cfg, max_depth, obstacles);
    
    float min_y = 1e9f, max_y = -1e9f;
    
    for (int i = 0; i < num_inputs_local; i++) {
        min_y = minf(min_y, input_y[i]);
        max_y = maxf(max_y, input_y[i]);
    }
    for (int i = 0; i < num_outputs_local; i++) {
        min_y = minf(min_y, output_y[i]);
        max_y = maxf(max_y, output_y[i]);
    }
    for (int i = 0; i < c->num_gates; i++) {
        if (!c->gates[i].alive) continue;
        min_y = minf(min_y, gate_y[i]);
        max_y = maxf(max_y, gate_y[i]);
    }
    
    float margin = cfg->min_gate_v_spacing * 2;
    min_y -= margin;
    max_y += margin;
    network->min_y = min_y;
    network->max_y = max_y;
    
    float scan_step = cfg->lane_spacing * 0.5f;
    float clearance = cfg->lane_spacing * 0.4f;
    float last_highway_y = min_y - cfg->min_gate_v_spacing;
    
    for (float y = min_y; y <= max_y && network->num_highways < MAX_HIGHWAYS; y += scan_step) {
        if (y - last_highway_y < cfg->lane_spacing * 2) continue;
        
        if (is_y_clear_globally(y, obstacles, max_depth + 1, clearance)) {
            Highway *hw = &network->highways[network->num_highways];
            hw->y = y;
            hw->lane_count = 0;
            for (int lane = 0; lane < 32; lane++) {
                hw->lane_offsets[lane] = (lane - 16) * HIGHWAY_LANE_WIDTH;
            }
            network->num_highways++;
            last_highway_y = y;
        }
    }
    ...
    if (network->num_highways < 4) {
        float emergency_positions[] = {
            min_y - margin,
            max_y + margin,
            (min_y + max_y) / 2.0f - margin,
            (min_y + max_y) / 2.0f + margin
        };
        
        for (int i = 0; i < 4 && network->num_highways < MAX_HIGHWAYS; i++) {
            Highway *hw = &network->highways[network->num_highways];
            hw->y = emergency_positions[i];
            hw->lane_count = 0;
            for (int lane = 0; lane < 32; lane++) {
                hw->lane_offsets[lane] = (lane - 16) * HIGHWAY_LANE_WIDTH;
            }
    ...
    for (int i = 0; i < network->num_highways - 1; i++) {
        for (int j = i + 1; j < network->num_highways; j++) {
            if (network->highways[j].y < network->highways[i].y) {
                Highway tmp = network->highways[i];
                network->highways[i] = network->highways[j];
                network->highways[j] = tmp;
            }
        }
    }
}

static int find_best_highway(
    HighwayNetwork *network,
    float src_y,
    float tgt_y,
    int wire_id
) {
    if (network->num_highways == 0) return -1;
    
    float min_y_wire = minf(src_y, tgt_y);
    float max_y_wire = maxf(src_y, tgt_y);
    
    int best_idx = 0;
    float best_score = 1e9f;
    
    for (int i = 0; i < network->num_highways; i++) {
        Highway *hw = &network->highways[i];
        float hw_y = hw->y;
        
        float vertical_travel = absf(src_y - hw_y) + absf(tgt_y - hw_y);
        
        float position_bonus = 0;
        if (hw_y >= min_y_wire && hw_y <= max_y_wire) {
            position_bonus = -2.0f;
        }
        
        float congestion_penalty = hw->lane_count * 0.1f;
        float score = vertical_travel + position_bonus + congestion_penalty;
...
static float allocate_highway_lane(Highway *hw, int wire_id) {
    int lane = (wire_id % 31);
    hw->lane_count++;
    return hw->y + hw->lane_offsets[lane];
}

static void write_wire_with_highways(
    FILE *f,
    ...
    int src_alley = (src_depth == -1) ? 0 : src_depth + 1;
    int tgt_alley = (tgt_depth == -2) ? grid->num_alleys - 1 : tgt_depth;
    
    if (src_alley < 0) src_alley = 0;
    if (tgt_alley >= grid->num_alleys) tgt_alley = grid->num_alleys - 1;
    
    int columns_to_cross = (tgt_alley > src_alley) ? (tgt_alley - src_alley) : (src_alley - tgt_alley);
    
    int lane_id = wire_id % 15;
    float alley_offset = (lane_id - 7) * cfg->lane_spacing;
    float step_out = 0.4f;
    
    if (columns_to_cross <= 1) {
        /* Short wire: direct routing */
        float alley_x = grid->alley_x[src_alley] + alley_offset;
        
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_x, src_y);
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_x + step_out, src_y);
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", alley_x, src_y);
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", alley_x, tgt_y);
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", tgt_x - step_out, tgt_y);
        fprintf(f, "        {\"x\":%.4f,\"y\":%.4f}\n", tgt_x, tgt_y);
    }
    else {
        /* Long wire: use highway */
        int hw_idx = find_best_highway(highways, src_y, tgt_y, wire_id);
        
        float src_alley_x = grid->alley_x[src_alley] + alley_offset;
        float tgt_alley_x = grid->alley_x[tgt_alley] + alley_offset;
        
        if (hw_idx < 0 || highways->num_highways == 0) {
            /* Fallback routing */
            float mid_y = (src_y + tgt_y) / 2.0f;
            ...
            /* Highway routing */
            Highway *hw = &highways->highways[hw_idx];
            float highway_y = allocate_highway_lane(hw, wire_id);
            
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_x, src_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_x + step_out, src_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_alley_x, src_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", src_alley_x, highway_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", tgt_alley_x, highway_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", tgt_alley_x, tgt_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f},\n", tgt_x - step_out, tgt_y);
            fprintf(f, "        {\"x\":%.4f,\"y\":%.4f}\n", tgt_x, tgt_y);
        }
...
/* ============================================================
 * END OF HIGHWAY ROUTING FUNCTIONS
 * ============================================================ */
...
void render_dls2_json(Circuit *c, int num_inputs_param, int num_outputs_param) {
    ...
    /* ========== NEW: Compute Highway Network ========== */
    HighwayNetwork *highways = (HighwayNetwork*)malloc(sizeof(HighwayNetwork));
    if (!highways) {
        printf("Error: Failed to allocate highway network\n");
        free(depths); free(gate_x); free(gate_y); free(gate_ids);
        free(input_ids); free(output_ids); free(input_y); free(output_y);
        fclose(f);
        return;
    }
    
    compute_highway_network(
        c, depths, gate_y,
        num_inputs_param, input_y,
        num_outputs_param, output_y,
        &cfg, max_depth, highways
    );
    
    printf("    [ROUTER] Found %d safe horizontal highways\n", highways->num_highways);
    /* ================================================== */
    ...
    float chip_width = (content_width / 10.0f) + cfg.chip_padding;
    float chip_height = (content_height / 10.0f) + cfg.chip_padding;
    chip_width = maxf(1.0f, minf(chip_width, 4.0f));
    chip_height = maxf(1.0f, minf(chip_height, 4.0f));
    ...
    /* ========== Write wires using NEW highway routing ========== */
    fprintf(f, "  \"Wires\":[\n");
    int wire_num = 0;
    int wire_id = 0;
    ...
    for (int i = 0; i < c->num_gates; i++) {
        ...
        if (g->op != EVO_NOT) {
            ...
            wire_num++;
            write_wire_with_highways(f, pin_b, owner_b,
    ...
    for (int i = 0; i < num_outputs_param; i++) {
        ...
        wire_num++;
        write_wire_with_highways(f, pin, owner, 0, output_ids[i],
