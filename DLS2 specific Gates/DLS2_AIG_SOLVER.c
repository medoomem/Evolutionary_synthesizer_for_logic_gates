...
#define MAX_AIG_NODES 200000
#define HASH_SIZE 65536       
#define MAX_GATES 100000      
#define MAX_EVOL_GATES 5000     
...
int evo_get_score(Circuit *c, BitVec *inputs, BitVec *targets, BitVec *masks, int num_chunks_param) {
    static BitVec wires[MAX_WIRES];  // <-- ADD static HERE
    static const BitVec zero = {0};
    
    // Clear wires before use (important when static!)
...
bool detect_bcd_to_7seg(TT **outputs, int n_in, int n_out) {
    ...
    for (int row = 0; row < 10; row++) {  /* Only check 0-9 */
        int input = 0;
        for (int i = 0; i < 4; i++) {
            input = (input << 1) | ((row >> (3 - i)) & 1);
        }
...
void build_bcd_to_7seg(Circuit *c) {
     ...
     * Inputs: A=0(MSB), B=1, C=2, D=3(LSB)
     * Outputs: a,b,c,d,e,f,g (active high)
     * 
    ...
    int B_notC_D = circuit_add_gate_alive(c, EVO_AND, B_notC, D);
    
    /* Segment a = A + C + B'D' + BD */
    int a_t1 = circuit_add_gate_alive(c, EVO_OR, A, C);
    int a_t2 = circuit_add_gate_alive(c, EVO_OR, notB_notD, BD);
    int seg_a = circuit_add_gate_alive(c, EVO_OR, a_t1, a_t2);
    
    /* Segment b = B' + C'D' + CD */
    int b_t1 = circuit_add_gate_alive(c, EVO_OR, notC_notD, CD);
    int seg_b = circuit_add_gate_alive(c, EVO_OR, notB, b_t1);
    
    /* Segment c = B + C' + D */
    int c_t1 = circuit_add_gate_alive(c, EVO_OR, B, notC);
    int seg_c = circuit_add_gate_alive(c, EVO_OR, c_t1, D);
    
    /* Segment d = A + B'D' + B'C + CD' + BC'D */
    int d_t1 = circuit_add_gate_alive(c, EVO_OR, A, notB_notD);
    int d_t2 = circuit_add_gate_alive(c, EVO_OR, notB_C, C_notD);
    int d_t3 = circuit_add_gate_alive(c, EVO_OR, d_t1, d_t2);
    int seg_d = circuit_add_gate_alive(c, EVO_OR, d_t3, B_notC_D);
    
    /* Segment e = B'D' + CD' */
    int seg_e = circuit_add_gate_alive(c, EVO_OR, notB_notD, C_notD);
    
    /* Segment f = A + C'D' + BD' + BC'D */
    int f_t1 = circuit_add_gate_alive(c, EVO_OR, A, notC_notD);
    int f_t2 = circuit_add_gate_alive(c, EVO_OR, B_notD, B_notC_D);
    int seg_f = circuit_add_gate_alive(c, EVO_OR, f_t1, f_t2);
    
    /* Segment g = A + B'C + BC' + CD' */
    int g_t1 = circuit_add_gate_alive(c, EVO_OR, A, notB_C);
    int g_t2 = circuit_add_gate_alive(c, EVO_OR, B_notC, C_notD);
    int seg_g = circuit_add_gate_alive(c, EVO_OR, g_t1, g_t2);
...
bool try_structural_synthesis(TT **outputs, Circuit *c) {
    ...
    if (detect_binary_to_bcd(outputs, num_inputs, num_outputs)) {
        printf("[*] Detected BINARY TO BCD!\n");
        /* Complex - would need Double Dabble builder */
        return false;
    }
...
int main() {
    ...
    if (try_structural_synthesis(outputs, &evo_circuit)) {
        ...
        /* If structural failed verification, fall back to AIG */
